...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function startQuiz() {
    ...
    resultContainer.classList.add('hidden');
    questionContainer.classList.remove('hidden');
    currentQuestionIndex = 0;  // Reset question index when starting/restarting
    score = 0;  // Reset score when starting/restarting
    showQuestion();
  }
  ...
  function showQuestion() {
    ...
    questions[currentQuestionIndex].choices.forEach(choice => {
      const li = document.createElement('li');
      li.textContent = choice;
      li.addEventListener('click', () => selectAnswer(choice, li)); // Pass the li element to highlight selection
      choicesList.appendChild(li);
    });
  }
